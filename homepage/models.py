...
class User(models.Model):
    ...
    photo = models.ImageField('Фото', blank=True, height_field="height",
                              width_field="width", upload_to=getpath)
...
class Task(models.Model):
    ...
    priority = models.IntegerField("Приоритет", choices=[(
        1, "High"), (2, "Middle"), (3, "Low")], default=1)
    deadline = models.DateField('Дата закрытия задачи')
    status = models.CharField(choices={
        "Todo": "Todo",
        "InProgress": "In progress",
        "Done": "Done"
    }, default="Todo", max_length=11)
    project = models.ForeignKey(
        Project, on_delete=models.CASCADE, related_name="tasks")
    creator = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="own_tasks")
